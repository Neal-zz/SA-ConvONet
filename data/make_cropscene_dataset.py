...
def make_temp_dataset(in_folder, out_folder):
    ...
    for pc_path in all_pc_list:
        ...
        # room9_noroof
        obj_name = os.path.basename(pc_path)[:-4]
        ...
        npz_dict = dict(points=points.astype(np.float32),
                        normals=normals.astype(np.float16),  # 无实际意义
...
if __name__ == "__main__":
    ...
    # data/yinshe_dataset_raw
    parser.add_argument('--in_folder', type=str, default="./data/yinshe_dataset_raw", help="the folder path containing point cloud ply files")
    # data/yinshe_dataset_normalize
    parser.add_argument('--out_folder', type=str, default="./data/yinshe_dataset_normalize", help="the output dir")
    args = parser.parse_args()

    make_temp_dataset(args.in_folder, args.out_folder)